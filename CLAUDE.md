# CLAUDE.md — lzt-flow project conventions

This file is read automatically by Claude Code. `AGENTS.md` in this repo carries the same
content in the tool-agnostic AGENTS.md format for other coding agents.
...
## Documentation style (README, docs/, любой .md для людей)
Пишется через скилл `/explane` (adhd-explainer) — доку читают прыжками, а не подряд.
Правила те же, если скилла под рукой нет:
- **Кратко. Без воды, без графоманства.** Каждое предложение либо несёт факт, либо удаляется.
  Абзац, пересказывающий следующий за ним код, — минус, а не плюс.
- **Одна мысль — одна строка**, пустая строка между строками. Не абзацы на 6 строк.
- **Ни одного нераскрытого термина.** Term появился → тут же, следующей строкой, вскрыт.
  Не можешь раскрыть без ухода в сторону — назови это отложенным чёрным ящиком явно.
- **Ловушку называй до объяснения**, а не после: «это **не** dry-run», «это **не** подпись».
  Иначе читатель уносит обе модели сразу.
- **Жирным — только несущие слова** (термины, якоря). Не целые предложения.
- Секция кончается вопросом, который открывает следующая. Стрелки и фрагменты вместо
  разъяснительной прозы.
- Произвольные факты (числа, точные имена, константы) помечай как «запомнить» — они не
  выводятся из логики.
- Никаких следов внутреннего процесса: имён приватных команд/скиллов, планов, спринтов,
  «перед подачей», приватности репозиториев, локальных путей рабочей машины.

## Задачи
- **Волна работ по итогам ревью фронтенда** — план `.plans/review-wave-01/`, чеклист
  `.plans/review-wave-01/00-prod-checklist.md`, выполнено **15/15**. **Волна 1 закрыта**
  (разбор `flowClient.ts` · пин `account_ref` · редизайн входа · скриншотный аудит).
  **Волна 2 закрыта**: редактор параметров `{{vars.X}}` обеими половинами (объявление +
  форма значений перед запуском) · undo/redo с двумя независимыми стеками · отказ публикации
  для обязательного параметра без дефолта у флоу с расписанием.
  Отчёты ревью — `../reviews/auto-lzt-*-2026-08-02.md`.
- **E2E-проверка на стенде** — план `.plans/e2e-testbox/00-plan.md`, отчёт
      `../reviews/auto-lzt-e2e-2026-08-02.md`. Стенд развёрнут, идут этапы E3–E5.
      **Найдено и починено 5 дефектов пути установки** (4 🔴): образ бэкенда не собирался без git;
      образ фронтенда — из-за порядка COPY в Dockerfile; воркер уходил в цикл перезапуска на
      конфигурации по умолчанию, из-за чего запуски не исполнялись вовсе; `.env` под Docker почти
      не действовал (в том числе настройка безопасности egress); конструктор нельзя было включить
      в образе. Правки — в ветке `e2e/waves-1-2`, не запушены.
- **Решения на потом** — `docs/decisions-later.md`, 1 запись.
