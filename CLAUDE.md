...
- **E2E-проверка на стенде** — план `.plans/e2e-testbox/00-plan.md`, не начато.
      Развернуть auto-lzt на сервере, провести один флоу через весь путь + 12 ударов на
      отказоустойчивость, затем повтор на боевом токене. Блокер записан в плане: выданный
      бокс занят тремя чужими стеками, свободно 701 МБ RAM на 1 ядре.
