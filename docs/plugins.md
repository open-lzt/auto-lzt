...
⚠️ Старая запись каталога без `sha256` не пройдёт валидацию — список доступных плагинов в боте
придёт **пустым**, без единой ошибки на экране. Ниже — что теперь обязано быть в записи.
...
референс — [`open-lzt/lzt-plugins`](https://github.com/open-lzt/lzt-plugins). Каждая запись:

- `name`, `version`, `description`;
- `source_url` — zip-архив, обязательно `https://`, хост — сам хост каталога либо
  `github.com` / `codeload.github.com` / `raw.githubusercontent.com` / `objects.githubusercontent.com`;
- `sha256` — **обязателен**: sha256 zip-архива, 64 hex-символа в нижнем регистре. Архив, чей
  хэш не совпал, отклоняется целиком — это то же самое сравнение, которым модульный реестр
  проверяет FLOW-модули, только здесь хэш защищает исполняемый код, а не данные;
- `requirements` — только точные пины `name==version` (extras разрешены), без URL, диапазонов,
  маркеров окружения и pip-опций (`-i`, `--find-links` и т.п.) — иначе `requirements` стал бы
  вторым каналом доставки кода в обход хэша и allow-list хостов.

Посчитать `sha256` архива:

```bash
sha256sum plugin.zip
```

Скачивание архива распаковывается с защитой от zip-slip и symlink-записей, а также от zip-бомбы —
распакованное дерево ограничено 64 МиБ. Приватный каталог — задайте `LZT_FLOW_PLUGIN_INDEX_TOKEN`
...
`manifest.name` в записанном при установке `manifest.json` обязан совпадать с именем папки плагина
и с именем, которое каталог назвал для этой записи — иначе установка отклоняется. И `name`, и
`entry` в манифесте проверяются по маске: `entry` — плоское имя `*.py` без разделителей и точек
для обхода вверх, потому что оно джойнится с папкой плагина и исполняется.
