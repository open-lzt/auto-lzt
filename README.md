...
Репозиторий, пакет и команда называются одинаково — `auto-lzt`. Старые имена `lzt-flow`, `lzt-flow-bot` и `lzt-flow-validate` остались рабочими псевдонимами, чтобы уже поставленные стенды и чужие CI не сломались.
...
Пакет ставит две команды: `auto-lzt` для операторских задач и `auto-lzt-validate` для проверки модуля.
